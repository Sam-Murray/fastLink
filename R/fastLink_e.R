...
#' fastLink_e
#'
#' Extended fastlink function for multiple metrics/cutoffs
#' 
#'
#' @usage fastLink(dfA, dfB, varnames, metrics
...
#' @param metrics A List of MetricBundle objects, the output of metricBundle(). Contains the metric function, names of variables that use that metric,
#' the cutoffs for that metric(sorted list of values), and the expected type of the input for the metric function(NULL if the metric can take any type).
...
#'
#' @return \code{fastLink_e} returns a list of class 'fastLink' containing the following components if calculating matches:
...
fastLink <- function(dfA, dfB, varnames,
  ...
  #gets list of names used for each metric
  metric_names = map(metric, ~.x$varnames)
  
  #Checks all names given to the metrics are in varnames
  if(any(!(union(metric_names) %in% varnames))){
    stop("You have provided a variable name for one or more of your metrics that is not in 'varnames'. Check your calls to bundleMetric to make sure all names passed to bundleMetric are in varnames.")
  }
  
  #Checks that names given to two distinct metrics are disjoint
  if(length(reduce(c,metric_names)) > length(union(metric_names))){
    stop("There is a variable that is assigned to two or more distinct metrics. Please select only one matching metric for each variable.")
  }
  ...
  ## Create mapping between metric and column(if the ith variable uses the jth metric, metric.ind[i] == j)

  get_metric_ind <- function(variablename, metrics){
    which(map_lgl(metrics, ~ variablename %in% .x))
  }
  
  metric.ind <-  map_int(varnames, get_metric_ind, metrics)
  ...
  for(i in 1:length(gammalist)){
    ...
    ## Get patterns
    if(metric.ind[[i]] != 0){
      gammalist[[i]] <-  metrics[[i]]$metricFunc(dfA[,varnames[i]], dfB[,varnames[i]], gender = gender.field[i], n.cores = n.cores)
    }else{
      gammalist[[i]] <- exactMetric(dfA[,varnames[i]], dfB[,varnames[i]], gender = gender.field[i], n.cores = n.cores)
    }
  }
  end <- Sys.time()
  if(verbose){
